...
#define TELEPHONY_PRIVILEAGE		"http://tizen.org/privilege/telephony"
#define TELEPHONY_ADMIN_PRIVILEAGE	"http://tizen.org/privilege/telephony.admin"
...
gboolean ps_util_check_access_control (cynara *p_cynara, GDBusMethodInvocation *invoc, const char *label, const char *perm)
{
	...
	if (g_strrstr(perm, PERM_WRITE) == NULL && g_strrstr(perm, PERM_EXECUTE) == NULL) {
		privilege = TELEPHONY_PRIVILEAGE;
	} else {
		privilege = TELEPHONY_ADMIN_PRIVILEAGE;
	}
